  ...
  return (
    <div className="mx-auto flex max-w-[1600px]">
      ...
      <aside
        ...
        className={`admin-sb fixed inset-y-0 left-0 z-50 flex w-64 shrink-0 flex-col border-r border-slate-200 bg-white shadow-xl transition-[transform,width] duration-200 lg:sticky lg:top-0 lg:z-auto lg:h-screen lg:translate-x-0 lg:shadow-none ${
          drawer ? "translate-x-0" : "-translate-x-full"
        } ${collapsed ? "lg:w-16" : "lg:w-60"}`}
      >
        <div
          className={`flex h-[4.25rem] shrink-0 items-center justify-between border-b border-slate-100 px-5 ${
            collapsed ? "lg:justify-center lg:px-0" : ""
          }`}
        >
          <Link href="/admin" className={`block ${collapsed ? "lg:hidden" : ""}`}>
          ...
          <button
            ...
            className={`-mr-2 flex h-8 w-8 cursor-pointer items-center justify-center rounded-lg text-slate-400 transition hover:bg-slate-100 hover:text-slate-800 ${
              collapsed ? "lg:hidden" : ""
            }`}
          >
          ...
          {/* Desktop rail only: expand back to the full sidebar. */}
          <button
            type="button"
            ...
            className={`hidden h-9 w-9 cursor-pointer items-center justify-center rounded-lg text-slate-500 transition hover:bg-slate-100 hover:text-slate-900 ${
              collapsed ? "lg:flex" : ""
            }`}
          >
      ...
      <div className="min-w-0 flex-1">
        ...
        <header className="sticky top-0 z-30 flex items-center gap-3 border-b border-slate-200 bg-white/95 px-4 py-2.5 backdrop-blur lg:hidden">
          <button
            ...
            className="-ml-1 flex h-9 w-9 cursor-pointer items-center justify-center rounded-lg text-slate-600 transition hover:bg-slate-100 hover:text-slate-900"
          >
            <Menu className="h-5 w-5" strokeWidth={2.2} />
          </button>
          <Link href="/admin" className="text-[14.5px] font-extrabold tracking-tight text-slate-900">
            Medico<span className="text-emerald-600">Bharat</span>
